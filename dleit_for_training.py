...
# This is only the version to be used for training
# this can (probably) be used in GPUs or whatever
# this will not (properly) compile for the TPU, though
# the TPU version will use an additional input, which is the embedding tokens
# since they are coded as weights here, they cannot be concatenated in the TPU
# (and we want to avoid sending to the CPU)
# so training will be done normally, and then the weights will be exported
class DLeiTForTraining(keras.Model):
	...
	def model(self, input_shape=None):
		...
		return keras.Model(
			inputs = input_pixels,
			outputs = self.call(input_pixels),
		)
	...
	def build(self, input_shape=None):
		...
		if getattr(self, "classifier", None) is not None:
			with tf.name_scope(self.classifier.name):
				#self.cls_classifier.build([self.config.batch_size, self.config.num_patches+2, self.config.hidden_size])
			...
			if getattr(self, "query", None) is not None:
				for q in self.query:
					with tf.name_scope(q.name):
						q.build([None, None, self.config.hidden_size])
			if getattr(self, "key", None) is not None:
				for k in self.key:
					with tf.name_scope(k.name):
						k.build([None, None, self.config.hidden_size])
			if getattr(self, "value", None) is not None:
				for v in self.value:
					with tf.name_scope(v.name):
						v.build([None, None, self.config.hidden_size])
	...
	def build(self, input_shape=None):
		...
		if getattr(self, "dropout", None) is not None:
			with tf.name_scope(self.dropout.name):
				self.dropout.build([None, None, self.hidden_size])
	...
	def build(self, input_shape=None):
		...
		if getattr(self, "projection", None) is not None:
			with tf.name_scope(self.projection.name):
				self.projection.build([None, *self.image_size, self.num_channels])
