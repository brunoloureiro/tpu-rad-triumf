...
# based on the Timm implementation of DeiT from
# https://github.com/huggingface/transformers/blob/696ededd2b8dbc6879aaa6a6ce2b287301ff7709/src/transformers/models/deit/modeling_tf_deit.py
...
#from transformers.models.deprecated._archive_maps import TF_DEIT_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
@dataclass
class TFDLeiTForImageClassificationWithTeacherOutput(ModelOutput):
	"""
...
class TFDLeiTPatchEmbeddings(keras.layers.Layer):
	"""
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTSelfAttention with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTSelfOutput with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTAttention with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTIntermediate with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTOutput with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTEncoder with ViT->DLeiT
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTPreTrainedModel with ViT->DLeiT all-casing
class TFDLeiTPreTrainedModel(TFPreTrainedModel):
	"""
...
DLEIT_INPUTS_DOCSTRING = r"""
	Args:
		pixel_values (`tf.Tensor` of shape `(batch_size, num_channels, height, width)`):
			Pixel values. Pixel values can be obtained using [`AutoImageProcessor`]. See
			[`DLeiTImageProcessor.__call__`] for details.
...
@add_start_docstrings(
	"The bare DLeiT Model transformer outputting raw hidden-states without any specific head on top.",
	DLEIT_START_DOCSTRING,
)
class TFDLeiTModel(TFDLeiTPreTrainedModel):
	def __init__(
		self, config: DeiTConfig, add_pooling_layer: bool = True, use_mask_token: bool = False, **kwargs
	) -> None:
		super().__init__(config, **kwargs)

		self.dleit = TFDLeiTMainLayer(
			config, add_pooling_layer=add_pooling_layer, use_mask_token=use_mask_token, name="dleit"
		)
...
# Copied from transformers.models.vit.modeling_tf_vit.TFViTPooler with ViT->DLeiT
class TFDLeiTPooler(keras.layers.Layer):
	def __init__(self, config: DeiTConfig, **kwargs):
		super().__init__(**kwargs)
...
@add_start_docstrings(
	"""
	DLeiT Model transformer with an image classification head on top (a linear layer on top of the final hidden state of
	the [CLS] token) e.g. for ImageNet.
	""",
	DLEIT_START_DOCSTRING,
)
class TFDLeiTForImageClassification(TFDLeiTPreTrainedModel, TFSequenceClassificationLoss):
...
@add_start_docstrings(
	"""
	DLeiT Model transformer with image classification heads on top (a linear layer on top of the final hidden state of
...
class TFDLeiTForImageClassificationWithTeacher(TFDLeiTPreTrainedModel):
	...
	@add_code_sample_docstrings(
		...
		output_type=TFDLeiTForImageClassificationWithTeacherOutput,
		config_class=_CONFIG_FOR_DOC,
	...
	def call(
		...
		return_dict: Optional[bool] = None,
		training: bool = False,
	) -> Union[tuple, TFDLeiTForImageClassificationWithTeacherOutput]:
		return_dict = return_dict if return_dict is not None else self.config.use_return_dict
		...
		return TFDLeiTForImageClassificationWithTeacherOutput(
			logits=logits,
